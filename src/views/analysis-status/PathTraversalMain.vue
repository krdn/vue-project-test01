<template>
  ...
  >
    <CRow class="double-contents-header">
      <CCol :xxl="12" :xl="12" class="container">
        <span class="title">{{ $t('PathTraversal') }}</span>
  ...
  >
    ...
    <AutoComplete
      ...
      width="250px"
      v-model="condition"
      :dataItems="searchCriteriaStore.getPathTraversalSearchCriterias"
    ></AutoComplete>
...
export default class PathTraversalMain extends Base {
  ...
  async created(): Promise<void> {
    ...
    this.searchCriterias.chart.type = ATTACK_TYPE.PATH_TRAVERSAL
    ...
    this.searchCriterias.grid.type = ATTACK_TYPE.PATH_TRAVERSAL
  ...
  saveButtonOnClick(e: Event) {
    this.searchCriteriaStore.pushPathTraversalSearchCriterias(this.condition)
  }
