<template>
  <div class="contents-wrapper" :class="{ 'no-display': detailView }" v-if="chartDataLoaded">
    <CRow class="double-contents-header">
      <CCol :xxl="12" :xl="12" class="container">
        <span class="title">{{ $t('XSS') }}</span>
    ...
    <CRow class="double-contents-body">
      <CCol>
        <BarChart
          ...
          :seriesItems="xssStatus!.seriesItems"
          :chartData="xssStatus!.data"
        ></BarChart>
      </CCol>
    </CRow>
    <CRow class="double-contents-header">
  ...
  >
    ...
    <AutoComplete
      ...
      width="250px"
      v-model="condition"
      :dataItems="searchCriteriaStore.getXSSSearchCriterias"
    ></AutoComplete>
...
export default class XSSMain extends Base {
  ...
  async created(): Promise<void> {
    ...
    this.searchCriterias.chart.type = ATTACK_TYPE.XSS
    ...
    this.searchCriterias.grid.type = ATTACK_TYPE.XSS
  ...
  saveButtonOnClick(e: Event) {
    this.searchCriteriaStore.pushXSSSearchCriterias(this.condition)
  }
