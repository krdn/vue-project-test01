<template>
  <div class="contents-wrapper" v-if="dataLoaded && searchPanelAPI != null">
    <CRow class="double-contents-header">
      <CCol :xxl="12" :xl="12" class="container">
        <span class="title">{{ $t('WebLogCollectStatusMain') }}</span>
    ...
    <CRow class="double-contents-body">
      <CCol>
        <LineChart
          :options="lineOptions"
          :seriesItems="webLogCollectStatus.seriesItems"
          :chartData="webLogCollectStatus.data"
        ></LineChart>
      </CCol>
    </CRow>
    <CRow class="double-contents-header">
    ...
    <CRow class="double-contents-body">
      <CCol>
        <DataGrid
          ...
          grid-data="/webLog/getWebLogCollectItems"
          export-data="/webLog/downloadWebLogCollectItems"
          :items-per-page="itemsPerPage"
          @grid-ready="gridOnReady"
          @grid-data-loaded="gridDataOnLoaded"
        >
        </DataGrid>
      </CCol>
    </CRow>
  ...
  >
    ...
    <AutoComplete
      ...
      width="250px"
      v-model="condition"
      :dataItems="searchCriteriaStore.getWebLogSearchCriterias"
    ></AutoComplete>
...
export default class WebLogCollectStatusMain extends Base {
  ...
  webLogCollectStatus: TChart = {} as TChart
  webLogCollectItems: any = {}
  ...
  gridOptions: TDataGridOptions = {
    ...
    columnDefinitions: [
      { field: 'rowNum', headerName: 'No', width: 85 },
      {
        field: 'start_date',
    ...
    detailColumns: ['raw_packet_data', 'start_date']
  }
  ...
  async created(): Promise<void> {
    ...
    await this.getWebLogCollectStatusData()

    this.dataLoaded = true
  }

  private async getWebLogCollectStatusData(): Promise<void> {
    const data = await this.$http.postAsync(
      '/webLog/getWebLogCollectStatus',
      this.searchCriterias.chart
    )

    this.webLogCollectStatus = data
  }
  ...
  saveButtonOnClick(e: Event): void {
    this.searchCriteriaStore.pushWebLogSearchCriterias(this.condition)
  }
