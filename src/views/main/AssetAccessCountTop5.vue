<template>
  <div class="dashboard-tile top-data">
    <div class="title">
      <span>{{ $t('AssetAccessCountTop5') }}</span>
    </div>
        ...
        <CTable hover>
          <CTableHead>
            <CTableRow>
              ...
              <CTableHeaderCell scope="col" v-html="$t('AssetIP')"></CTableHeaderCell>
              <CTableHeaderCell
                class="width-90 number-cell"
                scope="col"
                v-html="$t('AccessCount')"
              ></CTableHeaderCell>
              <CTableHeaderCell
                class="width-90 number-cell"
                scope="col"
          ...
          <CTableBody>
            <CTableRow v-if="tableItems.length > 0" v-for="(item, index) in tableItems">
              ...
              <CTableDataCell
                v-html="item.assetIP"
                @click="assetIPOnClick(item.assetIP)"
                class="ip-link"
              ></CTableDataCell>
              <CTableDataCell
                class="number-cell"
                v-html="item.accessCount.toLocaleString()"
              ></CTableDataCell>
              <CTableDataCell class="number-cell">
                <span
...
export default class AssetAccessCountTop5 extends Base {
  ...
  created(): void {
    this.tableItems = this.assetAccessCountTop5.lastWeekData
  }
  ...
  changeData(activeTab: number) {
    if (this.activeTab != activeTab) {
      ...
      if (activeTab == 1) {
        ...
        this.tableItems = this.assetAccessCountTop5.twoWeeksAgoData
      }
    }
  }
  ...
  assetIPOnClick(assetIP: string): void {
    this.$emit('detail-page-on-open', { srcIP: assetIP, detailView: true })
  }
}
</script>
