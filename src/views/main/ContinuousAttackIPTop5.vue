        ...
        <CTable hover>
          ...
          <CTableBody>
            <CTableRow v-if="tableItems.length > 0" v-for="(item, index) in tableItems">
              ...
              <CTableDataCell
                v-html="item.attackIP"
                @click="attackIPOnClick(item.attackIP)"
                class="ip-link"
              ></CTableDataCell>
              <CTableDataCell
                class="number-cell"
...
export default class ContinuousAttackIPTop5 extends Base {
  continuousAttackIPTop5!: any
  tableItems: Array<any> = []
  activeTab: number = 1

  created(): void {
    this.tableItems = this.continuousAttackIPTop5.lastWeekData
  }
  ...
  changeData(activeTab: number) {
    if (this.activeTab != activeTab) {
      ...
      if (activeTab == 1) {
        ...
        this.tableItems = this.continuousAttackIPTop5.twoWeeksAgoData
      }
    }
  }
  ...
  attackIPOnClick(attackIP: string): void {
    this.$emit('detail-page-on-open', { srcIP: attackIP, detailView: true })
  }
}
</script>
